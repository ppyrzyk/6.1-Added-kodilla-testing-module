...
//        Given
        Board project = prepareTestData();
//        When
        List<TaskList> averageOfIP = new ArrayList<>();
        averageOfIP.add(new TaskList ("In progress"));
        int sumOfDays = project.getTaskLists().stream()
                .filter(averageOfIP::contains)
                .flatMap(n -> n.getTasks().stream())
                .map(task -> task.getCreated().until(LocalDate.now()))
                .reduce((sum, current) -> sum = sum.plus(current)).get().getDays();
        long countOfIPTasks = project.getTaskLists().stream()
                .filter(averageOfIP::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(task -> task.getCreated())
                .count();
        double Average= (double)sumOfDays/(double)countOfIPTasks;
//        Then
        Assert.assertEquals(3, countOfIPTasks);
        System.out.println(countOfIPTasks);
        Assert.assertEquals(10.0, Average, 1);
...
//        Given
        Board project = prepareTestData();
//        When
        List<TaskList> averageInProgressTasks = new ArrayList<>();
        averageInProgressTasks.add(new TaskList( "In progress"));
        double average = project.getTaskLists().stream()
                .filter(averageInProgressTasks::contains)
